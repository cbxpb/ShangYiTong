...
<script setup lang="ts">
  ...
  // 存储医院科室挂号的数据
  const workData = ref<workData>({} as workData)
  // 存储排班日期:当前数据的第一个
  const workTime = ref<bookingSchedule>({} as bookingSchedule)
  // 存储排班医生的数据
  const doctorArr = ref<DoctorArr>([])
  // 医院的编号
  ...
  const fetchWorkData = async () => {
    let res: HospitalWordResponseData = await reqHospitalWork(
      pageNum.value,
      pageSize.value,
      hoscode,
      depcode
  ...
  const getDoctorWorkData = async () => {
    ...
    //获取排班医生的数据
    let res: DoctorResponseData = await reqHospitalDoctor(
      hoscode,
      depcode,
      workDate
    )
